...
def save_heatmap(matrix, xlabels, ylabels, title, path):
...
def make_performance_heatmap(metrics_path, fig_dir):
    ...
    save_heatmap(matrix, metric_cols, table["method"].tolist(), "Overall performance across predictive and legal-evidence metrics", os.path.join(fig_dir, "fig3_performance_heatmap.png"))
...
def make_rule_family_heatmap(pred, fig_dir):
    ...
    save_heatmap(pivot.values, pivot.columns.tolist(), pivot.index.tolist(), "Rule-family and proof-state performance", os.path.join(fig_dir, "fig4_rule_family_heatmap.png"))
